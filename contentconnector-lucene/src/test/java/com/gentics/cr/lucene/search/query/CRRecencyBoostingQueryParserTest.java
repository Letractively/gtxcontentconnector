...
public class CRRecencyBoostingQueryParserTest extends AbstractLuceneTest {
	...
	protected void setUp() throws Exception {
		...
		parser = new CRRecencyBoostingQueryParser(config, LuceneVersion.getVersion(), SEARCHED_ATTRIBUTES, STANDARD_ANALYZER, crRequest);
		...
		/* 3 */documents.add(new ComparableDocument(lucene.add(SimpleLucene.CONTENT_ATTRIBUTE + ":word1 daysAgo30", "updatetimestamp:"
				+ daysAgo30, "node_id:7")));
		...
		/* 5 */documents.add(new ComparableDocument(lucene.add(SimpleLucene.CONTENT_ATTRIBUTE + ":word1 word1 today", "updatetimestamp:"
				+ today, "node_id:1")));
		/* 6 */documents.add(new ComparableDocument(lucene.add(SimpleLucene.CONTENT_ATTRIBUTE + ":word1 daysAgo20", "updatetimestamp:"
				+ daysAgo20, "node_id:6")));
		/* 7 */documents.add(new ComparableDocument(lucene.add(SimpleLucene.CONTENT_ATTRIBUTE + ":word1 daysAgo10", "updatetimestamp:"
				+ daysAgo10, "node_id:4")));
		/* 8 */documents.add(new ComparableDocument(lucene.add(SimpleLucene.CONTENT_ATTRIBUTE
				+ ":word1 word1 word1 word1 word1 word1 word1 daysAgo30", "updatetimestamp:" + daysAgo30, "node_id:5")));
	}
	...
	public void testBoostingWithCRRecencyBoostingQueryParser() throws ParseException, CorruptIndexException, IOException {
		...
		assertEquals("Ordering of the matching collection not expected! First entry must be the 6th document.", i.next(), documents.get(5));
		assertEquals("Ordering of the matching collection not expected! Second entry must be the 1th document.", i.next(), documents.get(0));
		assertEquals("Ordering of the matching collection not expected! Third entry must be the 8th document.", i.next(), documents.get(7));
		assertEquals("Ordering of the matching collection not expected! Fourth entry must be the 9th document.", i.next(), documents.get(8));
		assertEquals("Ordering of the matching collection not expected! Fifth entry must be the 4th document.", i.next(), documents.get(3));
		assertEquals("Ordering of the matching collection not expected! Sixth entry must be the 7th document.", i.next(), documents.get(6));
	}
	...
	public void testCRRecencyBoostingQueryCalculation() throws ParseException, CorruptIndexException, IOException {
		...
		assertEquals("Comparison between the boost-calculating of the first document issn't correct!", result, testingResult);
	}
}
